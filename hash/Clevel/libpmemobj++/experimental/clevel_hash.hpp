	...
	namespace obj
	{
		namespace experimental
		{
			...
			template <typename Key, typename T, typename Hash = std::hash<Key>,
					  typename KeyEqual = std::equal_to<Key>, size_t HashPower = 19>
			class clevel_hash
			{
			public:
				using key_type = Key;
	 ...
	 */
				...
				KV_entry_ptr_t e1, KV_entry_ptr_t e2)
			{
				...
				if (tmp1_u.x.partial == tmp2_u.x.partial)
				{
					...
					else if (e1.get_address(my_pool_uuid)->first==
					...
					*e = nullptr;
					...
					for (size_type i = 0; i < n_levels; i++)
					{
						...
						for (size_type j = 0; j < assoc_num; j++)
						{
							...
							if (f_b.slots[j].x.partial != partial || !(f_e.get_address(my_pool_uuid)->first== key))
								continue;
							...
							if (result == FOUND_IN_LEFT || result == FOUND_IN_RIGHT)
							{
								...
								if (f_e.get_offset() == prev_e.get_offset())
								{
									...
									if (prev_i < i)
									{
										del_dup(pop, &f_b.slots[j], &(levels[level_num].get_address(my_pool_uuid)->buckets[idx].slots[slot_idx]), f_e, prev_e);
									}
									else
									{
										// Never fires!
						...
						for (size_type j = 0; j < assoc_num; j++)
						{
							...
							if (s_b.slots[j].x.partial != partial || !(s_e.get_address(my_pool_uuid)->first== key))
								continue;
							...
							if (result == FOUND_IN_LEFT || result == FOUND_IN_RIGHT)
							{
								...
								if (s_e.get_offset() == prev_e.get_offset())
								{
									...
									if (prev_i < i)
									{
										del_dup(pop, &s_b.slots[j], &(levels[level_num].get_address(my_pool_uuid)->buckets[idx].slots[slot_idx]), s_e, prev_e);
									}
									else
									{
										// Never fires!
							...
							if (tmp.x.partial == partial && tmp.p.get_offset() != 0)
							{
										...
										tmp.p.get_address(my_pool_uuid)->first== key))
								{
									if (CAS(&(f_b.slots[j].p.off), tmp.p.off, 0))
									{
										...
										if (m_copy != meta || (i == 0 && f_idx <= expand_bucket && f_idx >= expand_bucket_old))
										{
											continue;
										}
									}
							...
							if (tmp.x.partial == partial && tmp.p.get_offset() != 0)
							{
										...
										tmp.p.get_address(my_pool_uuid)->first == key))
								{
									if (CAS(&(s_b.slots[j].p.off), tmp.p.off, 0))
									{
										...
										// before deletion's CAS. Therefore, we can do context
										// checking to avoid such failures.
										if (m_copy != meta || (i == 0 && s_idx <= expand_bucket && s_idx >= expand_bucket_old))
										{
											continue;
										}
									}
				...
				while (true)
				{
					...
					if (result == FOUND_IN_LEFT || result == FOUND_IN_RIGHT)
					{
						...
						else if (CAS(&(e->off), old_e.raw(), created.p.raw()))
						{
							...
							if (m_copy != meta || (level_num == 0 && idx <= expand_bucket && idx >= expand_bucket_old))
							{
								succ_update = true;
								continue;
							}
				...
				while (run_expand_thread.get_ro().load())
				{
					...
					RETRY_REHASH:
						...
						for (size_type slot_idx = 0; slot_idx < assoc_num; slot_idx++)
						{
							...
							difference_type f_idx, s_idx;
							bool succ = false;
							hv_type hv = h(&e->first, sizeof(e->first));
							partial_t partial = get_partial(hv);
